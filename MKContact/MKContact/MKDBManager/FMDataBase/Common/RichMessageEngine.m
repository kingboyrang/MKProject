...
- (NSString *)createValues
{
    NSMutableString *cValueStr = [NSMutableString stringWithString:@""];
    [cValueStr appendString:@"("];
    [cValueStr appendFormat:@"%ld,",(long)self.msgId];
    ...
    [cValueStr appendFormat:@"%ld)",(long)self.imgIndex];
    return cValueStr;
}
...
+ (NSInteger)updateRichMessage:(NSArray *)msgList groupIsChanged:(BOOL)isChanged
{
    ...
    WldhDBManager *fmManager = [WldhDBManager shareInstance];
    ...
    if ([msgList count] > 0)
    {
        for (NSInteger i = 0; i < [msgList count]; ++i)
        {
            ...
            NSArray *result = [fmManager queryUserDBWithSql:kSQLRichMsgQueryOne,
                               [NSNumber numberWithInteger:oneMsg.msgId]];
            if (result.count > 0) {
                flag = NO;
            }
            if (flag) {
...
{
     WldhDBManager *fmManager = [WldhDBManager shareInstance];
    NSArray *result = [fmManager queryUserDBWithSql:kSQLRichContentSelectAllMsg];
    return result;
}

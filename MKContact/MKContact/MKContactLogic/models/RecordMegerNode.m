...
- (BOOL)isSameWithOther:(RecordMegerNode *)otherMegerRecord
{
    if (otherMegerRecord == nil)
    {
        return NO;
    }
    
    return [self.megerKey isEqualToString:otherMegerRecord.megerKey];
}
...
- (NSString *)megerKey
{
    ...
    {
        return [NSString stringWithFormat:@"%ld",(long)self.contactID];
    }
    else
    {

...
- (BOOL)addCommonContact:(NSInteger)contactID
{
    ...
    BOOL ret = NO;
    @synchronized([WldhDBManager shareInstance].globalDBQueue)
    {
        FMDatabaseQueue *dbHandle = [WldhDBManager shareInstance].globalDBQueue;
        if (nil != dbHandle)
        {
            ret = [[WldhDBManager shareInstance] updataGlobalDBWithSql:kSQLInsertTableCommonContact,
                   [NSNumber numberWithInt:contactID]];
        }
    }
    return ret;
...
- (BOOL)removeCommonContact:(NSInteger)contactID
{
    ...
    @synchronized([WldhDBManager shareInstance].globalDBQueue)
    {
        ...
        if (nil != dbHandle)
        {
            ret = [[WldhDBManager shareInstance] updataGlobalDBWithSql:kSQLDeleteCommonContact,
                   [NSNumber numberWithInt:contactID]];
        }
    }
...
//    [[FMDataBaseManager shareInstance] closeUserDatabase];
    ...
    @synchronized([WldhDBManager shareInstance].globalDBQueue)
    {
        ...
        if (nil != dbHandle)
        {
                NSArray *result = [[WldhDBManager shareInstance] queryGlobalDBWithSql:kSQLQueryAllCommonContact];

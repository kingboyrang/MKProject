...
- (void)loadDataWithFilePath:(NSString *)filePath
{
    ...
    [NSThread detachNewThreadSelector:@selector(loadDataThread:)
                             toTarget:self
                           withObject:filePath];
}
...
- (int) binarySearch:(int)phoneNumber;
{
    ...
    NSUInteger left    = 0;
    NSUInteger right   = [iRecordItemList count] - 1;
    NSUInteger middle  = 0;
...
//    NSString *strTemp = [NSString stringWithFormat:@"%@", strNumber];
    NSString *strTemp = [strNumber stringByReplacingOccurrencesOfString:@"+" withString:@""];
    strRet = strTemp;
...
{
#warning _bLoadDataFinished 全工程只有一次初始化，而且为NO
                ...
                if (aAttribution == [phoneTwo intValue])
                {
                    ...
                    if ([_chineseArealList count] > i+1)
                    {
                        ...
                        if (aAttribution == [[bkDic objectForKey: @"aAttribution"] intValue])
                        {
                            ...
                            NSUInteger l = [att rangeOfString:@" "].location;
                            if (NSNotFound != l)
                            {
